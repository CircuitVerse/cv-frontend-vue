...
export default function startListeners() {
  ...
  $("#projectName").on("click", () => {
    ...
    setTimeout(() => {
      (document.getElementById("projname") as HTMLInputElement).select();
    }, 100);
  });

  document.getElementById("simulationArea").addEventListener("mouseup", (_e) => {
    if (simulationArea.lastSelected) {
  ...
  window.addEventListener(
    ...
    (e) => {
      ...
      if (listenToSimulator) {
        ...
        // zoom in (+)
        if (
          (simulationArea.controlDown && (e.keyCode == 187 || e.keyCode == 171)) ||
          e.keyCode == 107
        ) {
          e.preventDefault();
          ZoomIn();
        ...
        // zoom out (-)
        if (
          (simulationArea.controlDown && (e.keyCode == 189 || e.keyCode == 173)) ||
          e.keyCode == 109
        ) {
          e.preventDefault();
          ZoomOut();
  ...
  $("#subcircuitMenu").on(
    ...
    function (this: HTMLElement, event: JQuery.Event, ui: any) {
...
function zoomSliderListeners() {
  ...
  function zoomSliderScroll(e: any) {
    ...
    if (zoomLevel >= 45) {
      ...
      (document.getElementById("customRange1") as HTMLInputElement).value = String(zoomLevel);
      curLevel = zoomLevel;
    }
  }
...
listen("new-project", () => {
  logixFunction.newProject(false);
});
