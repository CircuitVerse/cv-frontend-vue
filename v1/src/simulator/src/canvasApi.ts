...
function buildSelectionScope(selectedElements: any[]): { [key: string]: any[] } {
  ...
  selectedElements.forEach((el) => {
    updateOrder.forEach((type) => {
      if (Array.isArray((globalScope as any)[type]) && (globalScope as any)[type].includes(el)) {
        scope[type].push(el);
      }
    });
  });
...
export function moveTo(
  ...
  yy *= globalScope.scale;
  if (bypass) {
    ctx.moveTo(Math.round(xx + globalScope.ox + newX), Math.round(yy + globalScope.oy + newY));
  } else {
    ctx.moveTo(
...
export function fillText2(
  ...
  ctx.textAlign = "center";
  ctx.fillText(str, 0, Math.round(4 * globalScope.scale));
  ctx.restore();
}
...
export function fillText4(
  ...
  ctx.fillText(
    ...
    yy + y1 + globalScope.oy + Math.round((fontSize / 3) * globalScope.scale),
  );
}
