/**
 * Circuit auto-layout — simple layered layout (no external dependencies).
 *
 * Replaces dagre which crashes in Tauri production builds due to
 * CJS graphlib resolution failures (require('graphlib') || window.graphlib)
*/
var DEVICE_SIZES = {
 ...
 *
 * @param {object} circuitJSON - DigitalJS circuit with .devices and .connectors
 * @returns {object} Map of device ID → { x, y } positions
 */
export function computeLayout(circuitJSON) {
    if (!circuitJSON.devices || Object.keys(circuitJSON.devices).length === 0) {
        return {}
    }
