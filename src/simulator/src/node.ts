...
/**
 * The node types are published as globals because most element modules still
 * read them as bare identifiers.
 */
declare global {
  interface Window {
    NODE_INPUT: number;
...
/**
 * The parts of a Scope that a Node touches.
 */
interface Scope {
  allNodes: Node[];
  nodes: Node[];
...
/**
 * A Node's parent is a CircuitElement. TriState and ControlledInverter also own
 * a `state` node, which resolve() reads after checking objectType.
 */
interface NodeParent extends CircuitElement {
  state?: Node;
}

/**
 * The saved form of a Node, as produced by saveObject().
 */
interface NodeData {
  x: number;
  y: number;
 ...
 * @category node
 */
export function constructNodeConnections(node: Node, data: NodeData): void {
  for (var i = 0; i < data.connections.length; i++) {
    if (!node.connections.includes(node.scope.allNodes[data.connections[i]]))
...
function rotate(x1: number, y1: number, dir: string): [number, number] {
  if (dir == "LEFT") {
    return [-x1, y1];
  }
  if (dir == "DOWN") {
...
export function dec2bin(dec: number, bitWidth: number | undefined = undefined): string {
 ...
 * @category node
 */
export function loadNode(data: NodeData, scope: Scope): void {
  new Node(data.x, data.y, data.type, scope.root, data.bitWidth, data.label);
}
...
export default class Node {
  ...
  // Assigned from outside the constructor, so these are declared without an
  // emit to leave the runtime shape of a Node unchanged.
  declare oldx: number;
  declare oldy: number;
  declare deleted: boolean;
  declare showHover: boolean;
  declare verilogLabel: string;

  // Set on the prototype below.
  declare propagationDelay: number;
  declare cleanDelete: () => void;
  ...
  constructor(
    ...
    type: number,
    parent: NodeParent,
    bitWidth: number | undefined = undefined,
    label = "",
  ) {
    ...
    // time and index are only read back once the node has been queued, so
      ...
      if (this.type == NODE_OUTPUT && !this.subcircuitOverride) {
        if (this.parent.isResolvable() && !this.parent.queueProperties.inQueue) {
          if (
            ...
            // Guarded by the objectType checks above, so state is present.
  ...
  draw(): void {
    ...
    if (
      ...
      (this.isHover() && !simulationArea.selected && !simulationArea.shiftDown) ||
    ...
    if (this.hover || simulationArea.lastSelected == this) {
      if (this.showHover || simulationArea.lastSelected == this) {
        // engine.js declares canvasMessageData as a bare object literal, so its
        // shape has to be stated here.
        const messageData = canvasMessageData as {
          x: number;
          y: number;
          string: string | undefined;
        };
        messageData.x = this.absX();
        messageData.y = this.absY() - 15;
        if (this.type == NODE_INTERMEDIATE) {
          var v = "X";
          if (this.value !== undefined) {
          ...
          if (this.label.length) {
            messageData.string = `${this.label} : ${v}`;
          } else {
            messageData.string = v;
          }
        } else if (this.label.length) {
          messageData.string = this.label;
        }
      } else {
        setTimeout(() => {
  ...
  update(): void {
    ...
    if (!this.wasClicked && this.clicked) {
      ...
      if (
        this.prev == "a" &&
        distance(simulationArea.mouseX, simulationArea.mouseY, this.absX(), this.absY()) >= 10
      ) {
        if (
          Math.abs(this.x + this.parent.x - simulationArea.mouseX) >
          Math.abs(this.y + this.parent.y - simulationArea.mouseY)
      ...
      if (flag == 1) {
        ...
        if (n1 == undefined) {
          // x1 and y1 are undefined when prev is neither 'x' nor 'y'; the
          // constructor's isNaN guard handles that, as it did before.
    ...
    if (n == undefined) {
      for (var i = 0; i < this.parent.scope.wires.length; i++) {
        if (this.parent.scope.wires[i].checkConvergence(this)) {
          if (this.type != NODE_INTERMEDIATE) {
            const intermediate = new Node(this.absX(), this.absY(), 2, this.scope.root);
            this.connect(intermediate);
            this.parent.scope.wires[i].converge(intermediate);
          } else {
            this.parent.scope.wires[i].converge(this);
          }
          break;
        }
      }
  ...
  /**
   * The class body of this method used to be overwritten by an identical
   * assignment to Node.prototype, so only this version ever ran.
   */
  processVerilog(): void {
    if (this.type == NODE_INPUT) {
