...
export default class TB_Input extends CircuitElement {
   ...
   * @param {string} dir - direction.
   * @param {string} identifier - id to identify tests.
   * @param {JSON=} testData - input, output and number of tests.} x
   */
  constructor(
      x,
      y,
    ...
    this.outputs = [];
    this.running = false; // if tests are currently running.
    this.iteration = 0;
    this.setup();
  }
    ...
    for (var i = 0; i < this.testData.inputs.length; i++) {
      this.outputs.push(
          new Node(
              ...
              1,
              this,
              this.testData.inputs[i].bitWidth,
  ...
  customDraw() {
    ...
    fillText(
        ...
        xx + this.rightDimensionX / 2,
        yy + 14 + 10 + 20 * this.testData.inputs.length,
        10,
    );
    fillText(
        ctx,
