...
export function setupVerilogExportCodeWindow() {
  const myTextarea = document.getElementById('verilog-export-code-window');
...
export var verilog = {
  ...
  exportVerilog: function(scope = undefined) {
    ...
    for (id in scopeList) {
      ...
      for (let i = 0; i < scopeList[id].SubCircuit.length; i++) {
    ...
    for (const element in elementTypesUsed) {
    ...
    const report = this.generateReport(elementTypesUsed) + '\n';
    const testbench = this.generateTestBenchCode(DUTs);
  ...
  generateTestBenchCode: function(DUTs) {
    ...
    for (var i = 0; i < DUTs.length; i++) {
      ...
      const circuitName = sanitizeLabel(DUT.name);
      const dutHeader = this.generateHeaderHelper(DUT);
  ...
  exportVerilogScope: function(
    ...
    const res = this.processGraph(scope, elementTypesUsed);
  ...
  processGraph: function(scope, elementTypesUsed) {
    ...
    for (var i = 0; i < inputList.length; i++) {
      for (let j = 0; j < scope[inputList[i]].length; j++) {
  ...
  setLabels: function(scope = globalScope) {
    ...
    for (var i = 0; i < moduleList.length; i++) {
      ...
      for (let j = 0; j < scope[m].length; j++) {
