...
export function createCombinationalAnalysisPrompt(scope = globalScope) {
    ...
    $('#combinationalAnalysis').append(
        "<p>Enter Output names separated by commas: <input id='outputNameList' type='text'  placeHolder='eg. Out X, Out Y'></p>"
    )
    $('#combinationalAnalysis').append("<p style='text-align:center;'>OR</p>")
    $('#combinationalAnalysis').append(
        "<p>Enter Boolean Function: <input class='truth_table_input' autofocus id='booleanExpression' placeholder='Example: (AB)' type='text'></p>"
    )
    $('#combinationalAnalysis').append(
        "<label class='cb-checkbox'>I need a decimal column.<input id='decimalColumnBox' type='checkbox'></label>"
    )
