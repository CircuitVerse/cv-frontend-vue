<template>
  ...
  <div id="verilogEditorPanel" class="noSelect defaultCursor draggable-panel draggable-panel-css">
    <PanelHeader :header-title="$t('simulator.panel_header.verilog_module')" />

    <div class="panel-body">
      <div class="layout-body">
        <button class="largeButton btn btn-xs custom-btn--tertiary" @click="resetVerilogCode">
          {{ $t('simulator.panel_body.verilog_module.reset_code') }}
        </button>
        <button class="largeButton btn btn-xs custom-btn--primary" @click="saveVerilogCode">
      ...
      <div class="layout-body">
        <div>
          ...
          <select v-model="selectedTheme" class="applyTheme">
            <optgroup v-for="optgroup in Themes" :key="optgroup.label" :label="optgroup.label">
              <option v-for="option in optgroup.options" :key="option.value">
                {{ option.value }}
              </option>
            </optgroup>
          </select>
