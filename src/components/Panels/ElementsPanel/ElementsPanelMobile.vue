<template>
    <div
        ...
        :style="{
            bottom: simulatorMobileStore.showElementsPanel ? '0' : '-12rem',
        }"
    >
        <div class="panel-body" style="padding: 0">
            ...
            <div
                v-if="simulatorMobileStore.showCircuits === 'elements'"
                class="element-category-tabs"
            >
                <div
                    v-for="category in panelData"
                    :key="category[0]"
                    class="element-category-tab"
                    @mousedown="selectCategory(category[1], category[0])"
                    :style="{
                        backgroundColor:
                            selectedCategoryName === category[0]
                                ? 'var(--primary)'
                                : '#eee',
                        color:
                            selectedCategoryName === category[0]
            ...
            <div
                v-if="simulatorMobileStore.showCircuits === 'layout-elements'"
                class="element-category-tabs tabs-layout"
            >
                <div
                    v-for="(
                        group, groupIndex
                    ) in SimulatorState.subCircuitElementList"
                    :key="groupIndex"
                    class="element-category-tab"
                    @mousedown="
                        selectCategory(
                    ...
                    :style="{
                        backgroundColor:
                            selectedLayoutCategoryName === group.type
                                ? 'var(--primary)'
                                : '#eee',
                        color:
                            selectedLayoutCategoryName === group.type
...
onMounted(() => {
    ...
    if (SimulatorState.subCircuitElementList.length > 0) {
        selectedLayoutCategory.value =
            SimulatorState.subCircuitElementList[0].elements
        selectedLayoutCategoryName.value =
            SimulatorState.subCircuitElementList[0].type
    }
})

watch(
    () => simulatorMobileStore.showCircuits,
    () => {
        {
            if (simulatorMobileStore.showCircuits === 'layout-elements') {
                selectedLayoutCategory.value =
                    SimulatorState.subCircuitElementList &&
                    SimulatorState.subCircuitElementList.length > 0
                        ? SimulatorState.subCircuitElementList[0].elements
                        : []
                selectedLayoutCategoryName.value =
                    SimulatorState.subCircuitElementList &&
...
watch(
    ...
    (newVal) => {
        ...
        if (currSelectedType) {
            const newSelectedType = newVal.find(
                (typeGroup) => typeGroup.type === currSelectedType
            )
            selectedLayoutCategory.value = newSelectedType
                ? newSelectedType.elements
                : []
            selectedLayoutCategoryName.value = newSelectedType
                ? newSelectedType.type
                : null
        }
    }
)
...
const dragElement = (groupType: string | null, element: any, index: number) => {
    ...
    SimulatorState.subCircuitElementList.forEach((typeGroup) => {
        typeGroup.elements = typeGroup.elements.filter((_, elementIndex) => {
    ...
    const elIndex = SimulatorState.subCircuitElementList.findIndex(
        (typeGroup) => typeGroup.type === groupType
    )
...
const filteredElements = computed(() => {
    const elements = selectedCategory.value
...
const filteredLayoutElements = computed(() => {
    const elements = selectedLayoutCategory.value
        ? selectedLayoutCategory.value
        : SimulatorState.subCircuitElementList &&
          SimulatorState.subCircuitElementList.length > 0
        ? SimulatorState.subCircuitElementList[0].elements
...
.element-category-tabs::-webkit-scrollbar,
.mobile-icons::-webkit-scrollbar {
    height: 0px;
}

.element-category-tabs::-webkit-scrollbar-thumb,
.mobile-icons::-webkit-scrollbar-thumb {
    background-color: transparent;
    border-radius: 4px;
}

.element-category-tabs::-webkit-scrollbar-track,
.mobile-icons::-webkit-scrollbar-track {
    background-color: transparent;
}
