...
export function closeCircuit(circuitItem: CircuitItem): void {
  ...
  if (dependencies) {
    dependencies = `\nThe following circuits are depending on '${scopeList[circuitItem.id].name
      }': [ ${dependencies} ].\n Delete subcircuits of ${scopeList[circuitItem.id].name
  ...
  promptStore.DeleteCircuit.messageText = `Are you sure want to close: ${scopeList[circuitItem.id].name
...
export function deleteCurrentCircuit(
  ...
  const SimulatorState = <SimulatorStateType>useState()
  const circuitList = SimulatorState.circuitList
  ...
  const index = circuitList.findIndex(
    (circuit: any) => circuit.id === scope.id
  )
  circuitList.splice(index, 1)
