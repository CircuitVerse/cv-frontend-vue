<template>
  ...
  backgroundColor: '#555',
}" role="button" @click.prevent="onHeadingClick(index)">
            H{{ index }}
          </a>
        </div>
      </div>
      ...
      <div class="mode-toggle">
        <button :class="{ active: showSourceCode }" @click.prevent="toggleMode">
          <i class="fas fa-file-code" />
        </button>
        <button :class="{ active: fullscreen }" @click.prevent="toggleFullscreen">
          <i v-if="!fullscreen" class="fas fa-expand-arrows-alt" />
          <i v-else class="fas fa-compress-arrows-alt" />
        </button>
      </div>
    </div>

    <div v-if="showSourceCode">
      <textarea v-model="sourceCode" style="
                    width: 100%;
                    height: 13.85rem;
                    color: #fff;
                    padding: 1rem;
