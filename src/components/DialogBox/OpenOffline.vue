<template>
  <v-dialog v-model="SimulatorState.dialogBox.open_project_dialog" :persistent="false">
    <v-card class="messageBoxContent">
      <v-card-text>
        ...
        <div id="openProjectDialog" title="Open Project">
          <label v-for="(projectName, projectId) in projectList" :key="projectId" class="option custom-radio">
            <input type="radio" name="projectId" :value="projectId">
            {{ projectName }}<span />
            <i class="fa fa-trash deleteOfflineProject" @click="deleteOfflineProject(projectId)" />
          </label>
      ...
      <v-card-actions>
        <v-btn v-if="JSON.stringify(projectList) != '{}'" class="messageBtn" block @click="openProjectOffline()">
          open project
        </v-btn>
        <v-btn v-else class="messageBtn" block @click.stop="OpenImportProjectDialog">
          open CV file
        </v-btn>
      </v-card-actions>
    </v-card>
...
function openProjectOffline() {
  SimulatorState.dialogBox.open_project_dialog = false;
  const ele = $('input[name=projectId]:checked');
  if (!ele.val()) {
    return;
  }
