<template>
  <v-dialog v-model="SimulatorState.dialogBox.insertsubcircuit_dialog" :persistent="false">
    <v-card class="messageBoxContent">
      <v-card-text>
        ...
        <div id="insertSubcircuitDialog" class="subcircuitdialog" title="Insert SubCircuit">
          <template v-for="(value, scopeId) in scopeList" :key="scopeId">
            <label v-if="availableSubCircuits(value, scopeId)" class="option custom-radio inline">
              <input type="radio" name="subCircuitId" :value="scopeId" />
              {{ getName(value) }}
              <span></span>
            </label>
          </template>
      ...
      <v-card-actions>
        <v-btn v-if="flag == false" class="messageBtn" block @click="insertSubcircuit()">
          Insert SubCircuit
        </v-btn>
