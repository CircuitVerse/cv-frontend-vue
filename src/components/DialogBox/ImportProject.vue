<template>
  <v-dialog v-model="SimulatorState.dialogBox.import_project_dialog" :persistent="true">
    <v-card class="importProjectDialog">
      <v-text-field>
        ...
        <div v-cloak class="cloak" @drop.prevent="addDropFile($event)" @dragover.prevent>
          <v-file-input label="Click to Select or Drag and drop file here" class="fileInput" id="fileInput" center-affix
            :error-messages="errorMessage" max-errors="1" accept=".cv" v-model="file" prepend-icon="mdi-paperclip">
    ...
    parsedFileDate.scopes.forEach((scope: object) => {
      ...
      scopeSchema.forEach((key) => {
        if (!keys.includes(key)) {
          throw new Error('Invalid Scope data, missing key ' + key);
        }
      });
    })
    load(parsedFileDate)
