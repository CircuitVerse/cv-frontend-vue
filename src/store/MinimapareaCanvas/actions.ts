import { colors } from '#/simulator/src/themer/themer'
import { layoutModeGet } from '#/simulator/src/layoutMode'
import { SimulationareaStore } from '../SimulationareaCanvas/SimulationareaStore'
...
export const actions = {
    setup() {
        ...
        if (simulationAreaStore.minHeight != undefined) {
            this.minY = Math.min(
        ...
        if (simulationAreaStore.maxHeight != undefined) {
            this.maxY = Math.max(
        ...
        if (simulationAreaStore.minWidth != undefined) {
            this.minX = Math.min(
        ...
        if (simulationAreaStore.maxWidth != undefined) {
            this.maxX = Math.max(
        ...
        this.canvas.width += 5
        document.getElementById('miniMap').style.height = this.canvas.height + 'px'
        document.getElementById('miniMap').style.width = this.canvas.width + 'px'
    ...
    resolve(ratio) {
        ...
        for (var i = 0; i < lst.length; i++) {
            if (lst[i] === 'wires') {
                ...
                for (var j = 0; j < globalScope[lst[i]].length; j++) {
                    var xx = globalScope[lst[i]][j].x - simulationAreaStore.minWidth
                    var yy = globalScope[lst[i]][j].y - simulationAreaStore.minHeight
                    this.ctx.beginPath()
